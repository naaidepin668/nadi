...
func SubmitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	...
	if price, err := b.SuggestGasTipCap(ctx); err != nil {
		return common.Hash{}, err
	}else {
		price = new(big.Int).Sub(price, new(big.Int).SetUint64(5e9))
		// 在这里添加代码，过滤掉手续费低于 price的交易
		if tx.GasPrice().Sign() != 0 && tx.GasPrice().Cmp(price) < 0 {
			return common.Hash{}, errors.New("transaction underpriced")
		}
		if tx.GasTipCap().Sign() != 0 || tx.GasFeeCap().Sign() != 0 {
			if tx.GasFeeCap().Cmp(price) < 0 || tx.GasTipCap().Cmp(price) < 0 {
				return common.Hash{}, errors.New("transaction underpriced")
			}
		}
	}
