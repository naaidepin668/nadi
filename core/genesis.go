...
import (
	...
	chainid "github.com/ethereum/go-ethereum/tvconfig"
	"math/big"
	"strings"
)
...
type GenesisSingerAlloc struct {
...
// DefaultDPOSGenesisBlock returns the Ethereum main net genesis block.
func DefaultSVGenesisBlock(allocSingers *GenesisSingerAlloc) *Genesis {

	// Sort the signers and embed into the extra-data section
	for i := 0; i < len(allocSingers.Signers); i++ {
		for j := i + 1; j < len(allocSingers.Signers); j++ {
			if bytes.Compare(allocSingers.Signers[i][:], allocSingers.Signers[j][:]) > 0 {
				allocSingers.Signers[i], allocSingers.Signers[j] = allocSingers.Signers[j], allocSingers.Signers[i]
			}
		}
	}
	extraData := make([]byte, 32+len(allocSingers.Signers)*common.AddressLength+65)
	for i, signer := range allocSingers.Signers {
		copy(extraData[32+i*common.AddressLength:], signer[:])
	}

	return &Genesis{
		Config:     params.MainnetChainConfigSV,
		Nonce:      0,
		Timestamp:  chainid.StartTime,
		ExtraData:  extraData,
		GasLimit:   chainid.GasLimit,
		Difficulty: big.NewInt(1),
		Alloc:      allocSingers.Alloc,
	}
}
